...
from simpleQENS.materials.quinPF6 import transAndRotModel_2L
...
def make_global_model(temps, res, Qvalues, init_params):
    ...
    for temp in temps:
        ...
        g_params.add('fwhm_rot2_{}'.format(temp), min=0.0,
                     expr='fwhm_rot2_0 * exp(-11.6049669 * fwhm_rot2_Ea/({}))'.format(temp))
...
def get_fit(data, resolution, temps, init_params=None, init_fixes=None, minim_method='leastsq'):
    ...
    default_params = {'fwhm_rot_0': 0.1, 'fwhm_rot_Ea': 12.0,
                      'fwhm_rot2_0': 0.2,
                      'fwhm_rot2_Ea': -6.0,
                      'fwhm_trans_a_0': 0.2,
