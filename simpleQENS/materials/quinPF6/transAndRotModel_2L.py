# model with diffusion and rotation, now with two rotational processes
...
def generate_model_and_params_transRot(res_data, spectrum_index=0, init_vals=None):
    ...
    This model has 3 Lorentzians: one translational, and two translational + rotational.
    ...
    inelastic3 = LorentzianModel(prefix='l3_'+sp)
    ...
    mymodel = intensity * Convolve(reso, inelastic + inelastic2 + inelastic3) + background
    ...
    parameters['l3_' + sp + 'amplitude'].set(min=0.0001)
    parameters['l_'+sp+'sigma'].set(min=0.00001)
    parameters['l2_'+sp+'sigma'].set(min=0.00001)
    parameters['l3_' + sp + 'sigma'].set(min=0.00001)
    ...
    parameters['l3_' + sp + 'center'].set(expr='l_' + sp + 'center')
    parameters['l3_'+sp+'amplitude'].set(expr='1 - l2_'+sp+'amplitude - l_'+sp+'amplitude')

    # Some initial sensible values
    default_vals = {'I_'+sp+'c': 0.9, 'l_'+sp+'sigma': 0.04,
                    'l2_'+sp+'sigma': 0.04, 'l3_'+sp+'sigma': 0.04,
...
def make_global_model(res, Qvalues, init_params):
    ...
    g_params.add('fwhm_rot2', value=init_params['fwhm_rot2'], min=0.0)
    ...
    for i in range(0, n_spectra):
        ...
        ps['l3_{}_sigma'.format(i)].set(
            expr='0.50000*fwhm_rot2 + 0.50000*fwhm_trans_a*(1-sin(fwhm_trans_l*{})/({}*fwhm_trans_l))'.format(Qvalues[i],
                                                                                                             Qvalues[
                                                                                                                 i]))
...
def get_fit(data, resolution, init_params=None, init_fixes=None, minim_method='leastsq'):
    ...
    default_params = {'fwhm_rot': 0.1, 'fwhm_rot2': 0.01, 'fwhm_trans_a': 0.2,
                      'fwhm_trans_l': 1.5, 'I': 1.0,
                      }
